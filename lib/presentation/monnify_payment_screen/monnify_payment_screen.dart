...
import '../../services/monnify_services.dart';
...
import 'notifier/monnify_payment_notifier.dart';
...
class MonnifyPaymentScreenState extends ConsumerState<MonnifyPaymentScreen> {
  ...
  final MonnifyServices _monnifyServices = MonnifyServices();
  ...
  Future<void> _initializePayment() async {
    try {
      ref.read(monnifyPaymentNotifier.notifier).setLoading(true);
      debugPrint('MonnifyPaymentScreen: Initializing payment with:');
      debugPrint('  Amount: ${widget.amount}');
      debugPrint('  Email: ${widget.email}');
      debugPrint('  Reference: ${widget.reference}');

      final paymentUrl = await _monnifyServices.getPaymentUrl(
      ...
      debugPrint('MonnifyPaymentScreen: Payment URL received: $paymentUrl');

      if (paymentUrl.isEmpty) {
        final errorMsg = 'Payment URL is empty - backend may not have returned payment URL. Please check if:\n1. Backend API is running\n2. Monnify credentials are correct\n3. Network connection is stable';
        debugPrint('MonnifyPaymentScreen: ERROR - $errorMsg');
        
        ref.read(monnifyPaymentNotifier.notifier).setError(errorMsg);
        ref.read(monnifyPaymentNotifier.notifier).setLoading(false);
      ...
      ref.read(monnifyPaymentNotifier.notifier).setPaymentUrl(paymentUrl);
      ref.read(monnifyPaymentNotifier.notifier).setLoading(false);

      _initializeWebView(paymentUrl);
    } catch (e) {
      if (!mounted) return;
      debugPrint('MonnifyPaymentScreen: Exception during initialization: $e');
      
      ref.read(monnifyPaymentNotifier.notifier).setError('Error: $e');
      ref.read(monnifyPaymentNotifier.notifier).setLoading(false);
  ...
  void _initializeWebView(String paymentUrl) {
    try {
      ...
      ref.read(monnifyPaymentNotifier.notifier).setError('Error initializing payment: $e');
      Fluttertoast.showToast(
  ...
  void _handleUrlNavigation(String url) {
    if (url.contains('successful') || url.contains('reference=${widget.reference}')) {
      ref.read(monnifyPaymentNotifier.notifier).setPaymentSuccessful(
        true,
        widget.reference,
      );
  ...
  Widget build(BuildContext context) {
    final paymentState = ref.watch(monnifyPaymentNotifier);
